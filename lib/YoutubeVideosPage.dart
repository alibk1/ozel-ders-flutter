...
import 'package:ozel_ders/Components/YoutubeCard.dart';
...
class _YoutubeVideosPageState extends State<YoutubeVideosPage> {
  ...
  Future<void> loadInitialData() async {
    videosHolder = await _firestore.getAllVideos20Times();
    filterBlogs();
  }
  ...
  void filterBlogs() {
    ...
    List<Map<String, dynamic>> filteredVideos = videosHolder;

    if (searchText.isNotEmpty) {
      filteredVideos = filteredVideos.where((blog) {
        return blog['videoTitle']
            .toString()
            .toLowerCase()
    ...
    if (sortBy == 'date_desc') {
      ...
      filteredVideos.sort((a, b) => a['videoTitle'].toString().compareTo(b['videoTitle'].toString()));
    } else if (sortBy == 'title_desc') {
      filteredVideos.sort((a, b) => b['videoTitle'].toString().compareTo(a['videoTitle'].toString()));
    }
    ...
    if (startIndex >= filteredVideos.length) {
      ...
      if (endIndex > filteredVideos.length) {
        endIndex = filteredVideos.length;
      }
      videos = filteredVideos.sublist(startIndex, endIndex);
    }
                ...
                delegate: SliverChildListDelegate([
                  ...
                  GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return YoutubeCard(videoData: video);
                    },
                  ),
                  Padding(
                    padding: const EdgeInsets.symmetric(vertical: 16.0),
                    child: Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: (currentPage * blogsPerPage) < videosHolder.length
                              ? () {
                            setState(() {
                              currentPage++;
                              filterBlogs();
